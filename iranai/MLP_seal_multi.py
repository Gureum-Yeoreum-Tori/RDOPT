...
class ParametricMLP(nn.Module):
    """
    형상 파라미터와 1D grid(회전속도)를 입력으로 받아, 각 grid point별 스칼라 함수를 예측하는 점별(point-wise) MLP.
    outputs: [B, out_channels, n_vel]
    """
    def __init__(self, n_params, param_embedding_dim, hidden_channels, out_channels, n_layers):
        super().__init__()
        ...
        self.grid_encoder = nn.Sequential(
            nn.Linear(1, param_embedding_dim),
            nn.GELU(),
        )
        
        # point-wise trunk MLP (마지막 차원에 대해 동일 가중치 적용)
        trunk_layers = []
        in_dim = 2 * param_embedding_dim
        for _ in range(max(1, n_layers)):
            trunk_layers.append(nn.Linear(in_dim, hidden_channels))
            trunk_layers.append(nn.GELU())
            trunk_layers.append(nn.Dropout(0.05))
            in_dim = hidden_channels
        trunk_layers.append(nn.Linear(in_dim, out_channels))  # shared feature 채널
    ...
    def forward(self, params, grid):
        ...
        ge = self.grid_encoder(grid.view(B * L, 1)) # [B*L, emb]
        pe = pe.unsqueeze(1).expand(B, L, -1).reshape(B * L, -1)  # [B*L, emb]
        x  = torch.cat([ge, pe], dim=-1)            # [B*L, 2*emb]
        feat = self.trunk(x)                        # [B*L, out_channels]
        feat = feat.view(B, L, -1).permute(0, 2, 1) # [B, out_channels, L]
        return feat
    
class MultiHeadParametricMLP(nn.Module):
    """
    FNO 대신 점별(point-wise) MLP trunk + 채널별 1x1 head를 사용.
    기존 forward/출력 형태와 동일: outputs: [B, n_heads, n_vel]
    """
    def __init__(self, n_params, param_embedding_dim, hidden_channels, in_channels, n_heads, n_layers, shared_out_channels):
        super().__init__()
        shared_channels = shared_out_channels if shared_out_channels is not None else hidden_channels
        ...
        for _ in range(max(1, n_layers)):
            ...
            trunk_layers.append(nn.GELU())
            in_dim = hidden_channels
        trunk_layers.append(nn.Linear(in_dim, shared_channels))
        trunk_layers.append(nn.GELU())
        self.trunk = nn.Sequential(*trunk_layers)
        ...
        self.heads = nn.ModuleList([
            nn.Sequential(
                nn.Linear(shared_channels, shared_channels),
                nn.GELU(),
                nn.Linear(shared_channels, 1),
    ...
    def forward(self, params, grid):
        ...
        pe = pe.unsqueeze(1).expand(B, L, -1).reshape(B * L, -1)  # [B*L, emb]
        x = torch.cat([ge, pe], dim=-1)                      # [B*L, 2*emb]
        feat = self.trunk(x)                                 # [B*L, Csh]

        # 각 head를 적용해 [B, n_heads, L]로 조립
        outs = []
        for head in self.heads:
            y = head(feat)                                   # [B*L, 1]
            y = y.view(B, L).unsqueeze(1)                    # [B, 1, L]
            outs.append(y)
...
for mat_file in mat_files:
    ...
    model_save_path = os.path.join(base_dir, 'mlp_seal_best_multihead_'+mat_file+'.pth')
    network_path = os.path.join(base_dir, 'mlp_multihead_'+mat_file+'.pth')
    network_path_ts = os.path.join(base_dir, 'mlp_multihead_'+mat_file+'.pt')
    ...
    best_val_loss = float('inf')
    start_epoch = 0
    ckpt_best_path = os.path.join(base_dir, f'mlp_seal_best_multihead_{mat_file}.pth')
    ckpt_last_path = os.path.join(base_dir, f'mlp_seal_last_multihead_{mat_file}.pth')
