...
figsize_DC_one_third = (2.1, 1.8) # double column, big
...
def plot_unbalance_response_rated_speed(x_nodes, harmonic_resp, out_path, figsize=figsize_SC_sss, plot_rotor=False,
    ...
    Bx = np.stack([hx.real, -hx.imag], axis=-1)      # (..., 2)
    By = np.stack([hy.real, -hy.imag], axis=-1)      # (..., 2)
    B  = np.stack([Bx, By], axis=-2)                 # (..., 2, 2)

    U, S, Vt = np.linalg.svd(B)                      # batched SVD
    r_a = S[..., 0]                                    # 진짜 최대 진폭 (장반경)
    r_b = S[..., 1]                                    # 최소 진폭 (단반경)
...
def plot_unbalance_response_rated_speed_all(x_nodes, harmonic_resp, out_path, figsize=figsize_SC_sss,
    ...
    for ip in range(pop):
        ...
        A_signed = np.sqrt(r_a[ip,:]**2 + r_b[ip,:]**2) * np.sign(A_signed)
...
idx_x = np.arange(n_node) * 4
idx_y = idx_x + 2
hx = harmonic_init[0,0,idx_x,0]
hy = harmonic_init[0,0,idx_y,0]
...
a = np.sqrt( (xr**2 + xi**2 +yr**2 + yi**2)/2 + np.sqrt((xr**2 - xi**2 +yr**2 - yi**2)**2/4 + (xr*xi + yr*yi)**2) )
b = np.sqrt( (xr**2 + xi**2 +yr**2 + yi**2)/2 - np.sqrt((xr**2 - xi**2 +yr**2 - yi**2)**2/4 + (xr*xi + yr*yi)**2) )
...
# A_plot, _, _ = plot_unbalance_response_rated_speed(x_nodes, harmonic_resp=harmonic_init_oper, figsize=figsize_SC_ss, plot_rotor=True, rotor_elements=rotor_elements,added_elements=added_elements,brgs=brgs,seals=seals, out_path="ini_unb_shaft.png",ylim=25)
...
plot_2factor(F_pop=F_pop,F_par=F_pareto_corrected,ip=[0, 1],idx=sel,figsize=figsize_SC_one_third_ss, xlim=[17.5,24.5], ylim=[800,4000])
...
# labels = ['Ini.','1','2','3']
# plot_unbalance_response_rated_speed_all(x_nodes, harmonic_resp=harmonic_oper_sel, out_path=f'opt_unb_shaft_all.png', figsize=figsize_SC_s, plot_rotor=True, rotor_elements=rotor_elements,added_elements=added_elements,brgs=brgs,seals=seals,ylim=60,labels=labels,unb=unb,scale=50000)
...
def plot_unbalance_legend(ctx, out_path='unb_legend.png', figsize=figsize_DC, ncol=2):
    ...
    ax.legend(bbox_to_anchor=(0.5,0.5), loc='center',
              borderaxespad=0., ncol=ncol, frameon=False)
    ax.axis("off")
    fig.savefig(out_path, dpi=600, bbox_inches='tight')
    plt.close(fig)

plot_unbalance_legend(ctx,figsize=(2, 1.6))
...
def plot_bearing_id_hist(X_pop, X_par, n_brg, idx=None, figsize=figsize_SC_s):
    ...
    # idx_pop = X_pop[:, :2*n_brg].reshape(-1, n_brg, 2)[:,:,0].ravel()
    # idx_par = X_par[:, :2*n_brg].reshape(-1, n_brg, 2)[:,:,0].ravel()
    idx_pop = X_pop[:, [0,2]].ravel()
    idx_par = X_par[:, [0,2]].ravel()
    fig, ax = plt.subplots(figsize=figsize)
    ax.hist(idx_pop, bins=np.arange(1,55)-1, alpha=0.3, rwidth=0.8, align='mid',label='Cand.',color="#9467bd")
    counts, bins, patches = ax.hist(idx_par, bins=np.arange(1,55)-1, alpha=0.5, rwidth=0.8, align='mid',label='Pareto',color='#FB7AE1')
    ...
    boundaries = [1, 5, 6, 10, 16, 20, 24, 32, 40, 48]
    ax.set_xlim(0.1, 55.1)
    # ax.set_xticks(ticks=np.array(boundaries),labels=[])
    ax.set_xticks([])
    for x in boundaries:
        ax.text(x-0.5, -4, str(x), ha='center', va='top')
    # ax.text(np.array(boundaries),y = 0*np.array(boundaries),s=boundaries)
    ...
    if idx is not None:
        ...
        for i in range(len(idx)):
            ...
            ax.scatter(idx_sel[i]+0.5+pos,counts[idx_sel[i]],marker='*',s=65, c=c)
            
        #     # ax.scatter(idx_sel[i,0]+0.5,counts[idx_sel[i,0]],marker='*',s=40, c=c)
        # for i in range(len(idx)):
        #     c = colors[i % len(colors)]
        #     if i == 1:
        #         ax.scatter(idx_sel[i,1]+0.5,counts[idx_sel[i,1]],marker='*',s=40, c=c)
        #     elif i == 2:
        #         ax.scatter(idx_sel[i,1]+0.5,counts[idx_sel[i,1]],marker='*',s=40, c=c)
        #     else:
        #         ax.scatter(idx_sel[i,1]+0.5,counts[idx_sel[i,1]],marker='*',s=40, c=c)
        
    ax.set_xlabel('Bearing ID',labelpad=15); ax.set_ylabel('Count'); ax.legend(); fig.tight_layout(); 
    fig.savefig("bearing_hist.png", dpi=600, bbox_inches="tight")
    fig.show()


plot_bearing_id_hist(X_pop, X_pareto, n_brg, idx=sel, figsize=figsize_SC_ss)
