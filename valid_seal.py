...
for mat_file in mat_files:
    ...
    with h5py.File(mat_path, 'r') as mat:
            ...
            w_vec_ref = np.array(mat['params/wVec'])
            w_min = w_vec_ref[0,0]*30/np.pi
            w_max = w_vec_ref[0,-1]*30/np.pi
...
X = input_.transpose()
pop = X.shape[0]
rdc_flat  = model_seal.predict(3, X, w_vec).reshape(pop, 1, 4, n_w).squeeze()
rdc_true = rdc.transpose([2,0,1])

print(rdc_flat.shape)
print(rdc_true.shape)
